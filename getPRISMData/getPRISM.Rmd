...
For a recent project, I was interested in temperature-level data at the census tract level. I came across [the PRISM Climate Group](https://prism.oregonstate.edu/) at Oregon State. The only problem was, I needed data in tabular format, but their data are in raster format. I wrote a simple yet powerful R script that downloads a year's worth of daily temperature data and converts them to CSV files with tract-level weather data for the state of Washington. This article will walk through how to:
...
3. Convert raster data into CSV
   a. Bonus: I'll even explain what raster data are!

<br />

And, at the end, I will put it all together in a single script that you can use to download an entire year of temperature data.
...
Let's walk through all of that. First, we load rvest, the tidyverse, and other libraries we will be using. Then, we set the year of data we want. The very last step of this exercise is fairly power hungry, so let's prepare for parallel processing to speed things up. In this example, we are saving two cores. It's wise to save at least a couple of cores. I also wanted to specify some directories for all of my downloads and final files. To help keep my code organized and portable, I like to use a JSON config file. A config file is a good place to store the types of variables that would need to change to run your code on another system. It makes the code more readable, portable, and sharable. I also stored the path to my config files in an environmental variable, mostly for privacy reasons. My config file looks like this:
...
shp_reproj <- rgdal::spTransform(waCensus10, crs(myRast))
...
The scale appears to be correct, but there is one problem: the area covered by the raster (background image) is so much larger than our census tract borders. Having so much extra raster data would slow the next step down significantly. We use the crop solution to only keep the rectangle that contains our census tract borders:
...
And there you go! A single raster file, turned into a data frame. But....we have 365 raster files. Thankfully, we are all set up to do parallel processing. Let's turn our shape file re-project -> crop raster -> extract data process into a function. It will take some more work up front, because we need to programmatically figure out the date instead of hard-coding it. I also want to add some logic to save each result to file. Since there is a data refresh/revision schedule, it will be useful to keep the original data in separate CSVs. Finally, since this is just a demonstration, we will restrict the function to only run on the first 10 records.
